...
class Database:
    ...
    async def set_join_channel(self, id, channel):
        await self.col.update_one({'_id': int(id)}, {'$set': {'join_channel': channel}})

    async def get_join_channel(self, id):
        user = await self.col.find_one({'_id': int(id)})
        return user.get('join_channel', None)
